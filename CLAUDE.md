...
- Entries must never mention ephemeral task identifiers like `task-001`; describe the user-facing change instead so the log stays meaningful outside a single run.
- Version bumping logic:
  - MAJOR: Breaking changes (Removed, Deprecated)
  - MINOR: New features (Added, Changed)
  - PATCH: Bug fixes (Fixed, Security, Attempted)
- Update CHANGELOG.md when appropriate to. Use its current contents as a gauge for this decision.
- Always keep the most recent entry at the top of the file.
...
Never reference ephemeral task IDs (e.g., `task-001`) in CHANGELOG entries—describe the change in human terms instead.

Never modify files under `example-projects/`; only change orchestrator source/docs.

NEVER edit anything within example-projects/ . This dir is reserved for testing the orchestrator tool. Mistakes you find in this, must be viewed in a read-only capacity, with the understanding that they must be root-caused and then ensuring that the source code used to produce those erroneous values, must be fixed upstream so that they aren't produced again. That's it. Don't overthink it. You are forbidden from editing any file, ever, for any reason, within example-projects/

Always think about how we can fix problems be updating: 1) prompting of existing claude code calls within orchestrator, 2) adding new claude code to orchestrator, and/or 3) removing claude code calls from orchestrator. Always think of this before trying things like hardcoding fallbacks, regex, etc., where a claude code call update/addition/removal would make more sense. Think quality-first, rather than cost-first. Cost is typically net 0 for this tool since users will primarily use this tool via a rate limit-oriented subscription, rather than paying per-token.